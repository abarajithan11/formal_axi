...
module faxi_slave #(
	...
	reg	[F_LGDEPTH-1:0]		next_rd_nbursts, next_rd_outstanding;
	reg	[AW-1:0]		f_axi_wr_addr, wstb_addr;
	reg	[7:0]			f_axi_wr_len, val_wr_len;
	reg	[2:0]			f_axi_wr_size, f_axi_rd_cksize;
	reg	[1:0]			f_axi_wr_burst;
	wire	[2:0]			this_awsize;
	wire				wstb_valid;
	wire				i_active_lock, f_axi_ex_checklock, check_this_return;
	wire	[AW-1:0]		i_exlock_addr, f_axi_exreq_addr;
	wire	[7:0]			i_exlock_len, f_axi_exreq_len;
	wire	[1:0]			i_exlock_burst, f_axi_exreq_burst, f_axi_ex_state;
	wire	[2:0]			i_exlock_size, f_axi_exreq_size;
	wire				f_axi_exreq_return;
	wire	[IW-1:0]		f_axi_wr_checkid;

	assign this_awsize = f_axi_wr_size;
	assign i_active_lock = 1'b0;
	assign f_axi_ex_checklock = 1'b0;
	assign check_this_return = 1'b0;
	assign i_exlock_addr = '0;
	assign i_exlock_len = '0;
	assign i_exlock_burst = '0;
	assign i_exlock_size = '0;
	assign f_axi_wr_checkid = '0;
